...
class TestTree < Test::Unit::TestCase
  def test_tree_build
    ...
    decision_tree = DecisionTree.new(data: test_data)
    node = decision_tree.build_tree
  ...
  def test_build_subset_attrs
    ...
    decision_tree  = DecisionTree.new(data: test_data, use_random: true)
    node   = decision_tree.build_tree(test_data)
    ...
    decision_tree2  = DecisionTree.new(data: test_data, use_random: true)
    node2   = decision_tree2.build_tree(test_data)
  ...
  def test_categorize
    ...
    decision_tree = DecisionTree.new(data: test_data, use_random: false)
    decision_tree.build_tree(test_data)
    assert_equal(decision_tree.categorize([7,0,1,'McCarthy, Mr. Timothy J','male',54,0,0,17463,51.8625,'E46','S']), 'S', 'labeled set as S')
    assert_equal(decision_tree.categorize([2,1,1,'Cumings, Mrs. John Bradley (Florence Briggs Thayer)','female',38,1,0,'PC 17599',71.2833,'C85','C']), 'C', 'labeled set as C')
  end
  ...
  def get_splits(node)
    ...
    if ! node.is_leaf?
      splits.push(get_splits(node.left)) unless node.left
      splits.push(get_splits(node.right)) unless node.right
    end
